...
%\begin{itemize}
	...
	指定された処理対象の最終日付（を含めて、その日）から1週間前の日付を求め、その7日間の日付のリストdatelistを作っている
...
\begin{table}[h]
  ...
  \begin{tabular}{p{1.3cm}lp{8.5cm}}
		...
		ローカル & daily.py & 標準入力からの1日分のデータをグラフ化する（更新） \\
		ローカル & weekly.py & 標準入力からの1週間分のデータをグラフにする（更新） \\
		ローカル & dayplot.sh & mycloud.py から daily.py へパイプで繋いでいる \\
		ローカル & weekplot.sh & mycloud.py から weekly.py へパイプで繋いでいる \\
		ローカル & myplot.py & daily.py,weekly.py,cloudplot.pyから呼ばれる（追加更新）\\
...
\begin{verbatim}
			...
			startstr = (today - timedelta(days=6)).strftime('%Y-%m-%d')
			datelist, list0 = [], []
			for i in range(7):
			   day = today - timedelta(days=(6-i))
...
\begin{verbatim}
  ...
  例3 mat@raspi:~/Documents $ sh ./cloudplot.sh 6
  例4 mat@raspi:~/Documents $ sh ./cloudplot.sh 1 2024-05-10
  例5 mat@raspi:~/Documents $ sh ./cloudplot.sh 6 2024-05-10
  例6 mat@raspi:~/Documents $ sh ./cloudplot.sh 10 2024-05-10
  例7 mat@raspi:~/Documents $ sh ./cloudplot.sh 30 2024-05-10 4 5
\end{verbatim}
\end{screen}

\begin{itemize}
\item[例1は] 実行日の前日の1日分のデータをグラフにする（例2も同じ指示になる）
\item[例3は] 実行日の前日から遡って6日分のデータをグラフ化する（２行3列で）
\item[例4は] 指定日付2024-05-10の、1日分のデータをグラフ化する
\item[例5は] 指定日付2024-05-10から遡って6日分のデータをグラフ化する（２行3列で）
\item[例6は] 指定日付前の6日間を、２行3列でグラフ化する（10日分の表示はしない）
\item[例7は] 指定日付前の20日間を、4行5列でグラフ化する（30日分の表示はしない）
\end{itemize}
\end{spacing}
...
dayplot.sh、weekplot.sh、cloudplot.sh に共通することとして

\begin{itemize}
\item プログラム実行日当日分のデータは、まだクラウド上にない（昨日分までがクラウド上にある）ので、「本日以降の日付」が指定された場合は、昨日の日付に解釈し直している
\item 「本日以降の日付」かどうかは、Unix時間で比較している.\\
dateコマンドで日付時間をUnix時間に直すことができる（ -d あるいは -\;-date オプションを指定し、Unix時間として出力するための書式文字には \%s を指定する）
\item なお、shell上での加算減算は、\$((\$a+\$b))などとすることもできる
\end{itemize}
...
\begin{figure}[htbp]
	\begin{minipage}[b]{1.0\linewidth}
		\centering
		\includegraphics[keepaspectratio, width=200mm, angle=90]{figs/png/savefig2.png}
		\caption{./cloudplot.sh 24 2024-05-30 6 4}
	\end{minipage}
	%\hspace{1.0cm}
	%\begin{minipage}[b]{0.45\linewidth}
	%\centering
...
remoteマシンが1日分のデータをクラウド上にアップロードする際に、100日より以前のクラウド上のデータは削除する処理を upcloud.py に追加した。

\begin{lstlisting}[caption=upcloud.py]
  from datetime import datetime as dt, timedelta
  import sys, pandas as pd
  from gcloud import GCSWrapper
  ...
  class cloud(GCSWrapper):
    def delete_blob(self, blob_name):
        """https://cloud.google.com/storage/docs/samples/storage-
           delete-file?hl=ja#storage_delete_file-python"""
