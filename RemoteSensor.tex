...
\begin{itemize}
	...
	本システムの中で、Pythonの仮想環境に移る必要性からbashで動作させなければならないshellは、ローカルマシン上のdaily.sh、weekly.sh、lack.sh、rtplot.shの4つ. またリモートマシンのsensor.shは、プロセス置換を実施するためにbashが必要になった. それ以外のshellは/usr/bin/shで動作する（全部bashにしてしまっても良かったかも）
