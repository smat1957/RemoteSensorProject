...
do
  ...
  if [ -n "$match" ]; then
    ...
    sshpass -p "${PASS}" ssh "${USR}"@"${ADDR}" \
      cat "${DIR}"/w.txt >> "${DIR}"/ww.txt
    sort -u "${DIR}"/ww.txt > ${DIR}/www.txt
    rm ${DIR}/ww.txt ${DIR}/w.txt
    mv ${DIR}/www.txt ${DIR}/w.txt
  else
    RC=$(sshpass -p ${PWRD} ssh ${USR}@${ADDR} \
      'test -f ${DIR}/data/${LINE};echo $?')
    if [ ${RC} -eq 0 ]; then
      sshpass -p "${PWRD}" ssh "${USR}"@"${ADDR}" \
        cat "${DIR}"/data/"${LINE}" > "${DIR}"/data/"${LINE}"
    fi 
  fi 
done < ${FILE_NAME}
	\end{verbatim}
