...
\begin{itemize}
	...
	\item ローカルマシン上に蓄積されたデータは、適切なタイミングでクラウド上のGCS（google-cloud-storage）へ退避させ、ローカルマシンの補助記憶容量を圧迫することがない様にする
\end{itemize}
...
upload0.py は、google-cloud-storageへの、その日のデータのアップロード（詳細は後述）

%\lstinputlisting[caption = upload.py, label = prog3]{src/remote/upload.py}
...
\item ユーザ認証情報とサービス アカウント認証情報を上の手続きで提供できたなら、以下の場所に必要な情報が保存されているので、他のマシンでgoogle-cloud-storage のAPIを使用したプログラムを動かしたい場合、〜/.config/gcloudのフォルダごと同じ場所にコピーすれば、容易に実行させることができる
\begin{verbatim}
...
このラッパークラスは、リモートマシンの日毎のデータをGCSにアップロードする際にも使っている

以下は、GCSの操作\footnote{https://cloud.google.com/storage/docs/samples/?hl=ja}を試行したもの
\begin{itemize}
...
\lstinputlisting[caption = mycloud.py, label = prog2]{src/local/mycloud.py}
...
\begin{itemize}
	...
	\item Code samples with GCS API\\
