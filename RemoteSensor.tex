...
\begin{verbatim}
  ...
  例5 mat@raspi:~/Documents $ sh ./cloudplot.sh 7 2024-05-10 2 4
\end{verbatim}
\end{screen}
\end{spacing}
...
\item[例5は] 9日分のデータを２行4列でグラフ化する（8日分しか表示しない）
\end{itemize}
...
myplot.py をこれまでのものと書き換えている

graph\_plot()は1つのグラフ用紙に描画するが、graph\_plots()はnrows=rows行ncols=cols列のグラフ用紙に描画する（rows=cols=1の場合、graph\_plots()でも作図可能だが、graph\_plot()も残している）
