...
\begin{itemize}
	...
	\item コマンドとして実行した結果の文字列が必要なら、アクサングラーブ（バッククゥオート）でコマンドを囲むか、あるいは\$(command)の形にする（アクサングラーブはシングルクゥオートと見分けにくい）例えば、次の\${match}ではTODAYをダブルクゥオートで囲んでいるが失敗する（TODAYをシングルクゥオートで囲むと当然失敗する）
	\begin{verbatim}
	TODAY="date +%Y-%m-%d"
	match=$(echo "${LINE}" | awk "/^${TODAY}/")
	\end{verbatim}
