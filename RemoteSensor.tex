...
\begin{itemize}
	\item \$\{LINE\}と\$\{match\}は、ダブルクゥオーテーションで囲む必要がある	
	\begin{verbatim}
		match=$(echo "${LINE}" | awk "/^$searchstring/")
		if [ -n "${match}" ]; then
	\end{verbatim}
	\item shellの変数名をダブルクオートで囲む必要がある場合とはどんな場合に必要なのか？不明（ダブルクオートで囲まないと評価してくれないことがある）
	\item if文の -n は、"\$\{match\}"が空でないならばという意味（空ならばは、-z ）\\マッチした場合は、マッチした文字列が"\$\{match\}"に入ってくる
	\item 予めCMD変数に、\$\{LINE\}と\$\{DIR\}の評価を済ませておかなければならない
