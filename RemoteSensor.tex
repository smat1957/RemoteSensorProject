...
\begin{itemize}
	...
	\begin{verbatim}
		...
		if [ -n "${match}" ]; then
	\end{verbatim}
	\item 変数TODAYの様に、dateの様な「コマンドを納めている変数」の場合、ダブルクゥオートで挟んだ変数"\$\{TODAY\}"に出会ったタイミングで、その中のコマンドが実行され、実行結果として得られた文字列が使える様になるということ
	
	ダブルクゥオートで挟むことは「中のコマンドを実行して評価して下さい」という意味（変数をダブルクゥオートで挟まなかったら、その変数の中のコマンドをシングルクゥオートで挟んでいた場合と同じ、単なる文字列として扱われる）
