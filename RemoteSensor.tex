...
usr='mat'
pass='mypassword'
sshpass -p ${pass} ssh ${usr}@${addr} ${dir}/sensor.sh \
 >> ${dir}/w.txt
	\end{verbatim}
\end{itembox}
...
ローカルに無い日付のファイル名が分かったので、
それをリモートからとってきてローカルに保存するshell を実行する
...
do
  ...
  if [ -n "${match}" ]; then
    ...
    sshpass -p ${PWRD} ssh ${USR}@${ADDR} cat ${DIR}/w.txt \
      >> ${DIR}/ww.txt
    ...
    if [ ${RC} -eq 0 ]; then
      sshpass -p ${PWRD} scp ${USR}@${ADDR}:${DIR}/data/${LINE}\
        ${DIR}/data/${LINE}
    fi
  fi
done < ${FILE_NAME}
...
\newpage
ただしプログラムを実行した当日のデータについては、ローカルマシンとリモートマシンの両方にある可能性があるので、両方をマージしてローカルマシン上のデータとした
...
\begin{itemize}
	...
	\begin{verbatim}
		...
		if [ -n "${match}" ]; then
	\end{verbatim}
	\item shellの変数名をダブルクオートで囲まないと期待した評価がなされないことがある（単なる文字列になっていて、それを評価した結果の文字列が必要ならばダブルクオートで囲む必要があるということ）\\例えば、次のケースでは\$\{TODAY\}は"date +\%Y-\%m-\%d"とダブルクオートで囲んでいるため、評価済みの文字列が\$\{TODAY\}に入っており、searchstring=\$\{TODAY\}.txtの形でOk
	\begin{verbatim}
TODAY="date +%Y-%m-%d"
searchstring=${TODAY}.txt
	\end{verbatim}
一方、次の様にシングルクオートで囲んでいる場合、\$\{TODAY\}は単に指定した文字列そのものなので、その文字列は評価される必要があって、searchstring="\$\{TODAY\}".txtの様にダブルクオートで囲んで評価させることになる
\begin{verbatim}
	TODAY='date +%Y-%m-%d'
	searchstring="${TODAY}".txt
\end{verbatim}
